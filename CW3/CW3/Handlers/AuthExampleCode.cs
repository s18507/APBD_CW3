    ...
    public class AuthExampleCode
    {
        public class BasicAuthHandler : AuthenticationHandler<AuthenticationSchemeOptions>
        {
            ...
            protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
            {
                ...
                var credentials = Encoding.UTF8.GetString(credentialBytes).Split(":"); //loginh:haslo

                if (credentials.Length != 2)
                    return AuthenticateResult.Fail("Incorrect authorization header value");

                if (credentials[1] != "asd123")
                    return AuthenticateResult.Fail("Incorect password");
